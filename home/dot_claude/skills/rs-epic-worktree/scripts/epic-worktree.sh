...
_create_wt() {
	...
	local ok=true err_msg=""
	if [[ -n "$cmd" ]]; then
		# shellcheck disable=SC2086
		# Run from MINIONS_HOME parent (ai-tools root) so mise activates the correct Ruby.
		# eval expands env vars (e.g., $MINIONS_HOME) in worktree_cmd.
		local run_dir="${MINIONS_HOME:+${MINIONS_HOME%/scripts/minions}}"
		local err_file
		err_file=$(mktemp)
		(${run_dir:+cd "$run_dir" &&} eval $cmd --repo '"$gs"' --branch '"$branch"') >/dev/null 2>"$err_file" || ok=false
		if [[ "$ok" != "true" ]]; then
			err_msg=$(head -5 "$err_file" | tr '"' "'" | tr '\n' ' ')
			rm -f "$err_file"
			printf '{"repo":"%s","id":"%s","branch":"%s","path":"unknown","type":"%s","status":"failed","error":"%s"}\n' \
				"$repo" "$id" "$branch" "$type" "$err_msg" >"$out"
			return 1
		fi
		rm -f "$err_file"
	else
...
_setup_repo() {
	...
	# Epic worktree
	_create_wt "$GS" "$WT" "$epic" "$FB" "$WCMD" "$out_dir/$idx.json" "$repo" "epic" || return 1
	idx=$((idx + 1))
	...
	while IFS=$'\t' read -r tid desc; do
		[[ -z "$tid" ]] && continue
		_create_wt "$GS" "$WT" "$tid" "$(branch_name "$epic" "$tid" "$desc")" "$WCMD" "$out_dir/$idx.json" "$repo" "ticket" || return 1
		idx=$((idx + 1))
	done <<<"$tickets"
...
cmd_setup() {
	...
	# Surface errors to stderr so the caller can see what went wrong
	if [[ $failed -gt 0 ]]; then
		for f in "$results_dir"/*/*.json; do
			[[ -f "$f" ]] || continue
			local err
			err=$(jq -r 'select(.status == "failed") | "\(.repo)/\(.id): \(.error // "unknown error")"' "$f" 2>/dev/null)
			[[ -n "$err" ]] && echo "FAILED: $err" >&2
		done
		echo "" >&2
		echo "worktree_cmd failed. No worktrees were created for the affected repo(s)." >&2
		echo "Fix the issue and re-run, or remove worktree_cmd from the manifest to use plain git worktree add." >&2
	fi
